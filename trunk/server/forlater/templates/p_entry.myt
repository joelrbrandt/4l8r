...
% if c.e['snippet_time']:
<h1>Your Entry - <% c.e['snippet_time'].strftime(g.long_datetime_format) %></h1>
% else:
<h1>Your Entry</h1>
%
...
<param name='movie' value='/mp3player.swf?mp3URL=/p/audio/<% snip["id"] %>.mp3&when=<% c.e['snippet_time'].strftime(g.short_datetime_format) %>' /><param name='quality' value='high' /><param name='bgcolor' value='#ffffff' />
<embed src='/mp3player.swf?mp3URL=/p/audio/<% snip["id"] %>.mp3&when=<% c.e['snippet_time'].strftime(g.short_datetime_format) %>' quality='high' bgcolor='#ffffff' width='330' height='125' name='mp3player' align='middle' allowScriptAccess='sameDomain' type='application/x-shockwave-flash' pluginspage='https://www.macromedia.com/go/getflashplayer' />
</object>
%   elif snip['type'] == 'picture':
<p><img src='/p/picture/<% snip["id"] %>.jpg'/></p>
%   else:
...
<h2>Questions</h2>
<p>Please respond to the following questions. Questions you have not yet answered are shown in red.</p>

<form action='/p/submit/<% c.id %>' method="POST">
...
<p><a href="/p">Cancel, and go back to main page.</a></p>
